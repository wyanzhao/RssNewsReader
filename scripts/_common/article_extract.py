...
class _MainTextParser(HTMLParser):
    ...
    def __init__(self) -> None:
        ...
        # Full tag stack so we can correctly decrement `_container_depth`
        # when a container-opening tag closes — including `role="main"` on
        # arbitrary tags like `<section>` / `<div>`, where the closing tag
        # name does not match PREFERRED_CONTAINERS on its own.
    ...
    def handle_starttag(self, tag: str, attrs) -> None:
        ...
        is_container = tag in PREFERRED_CONTAINERS or role == "main"
        if is_container:
    ...
    def handle_endtag(self, tag: str) -> None:
        ...
        # Pop the innermost matching tag off the stack. Walking backwards
        # tolerates malformed HTML where inner tags were never explicitly
        # closed — any orphaned entries above the match are discarded but
        # their container-depth contributions are unwound too.
        for idx in range(len(self._tag_stack) - 1, -1, -1):
            if self._tag_stack[idx][0] == tag:
                dropped = self._tag_stack[idx:]
                del self._tag_stack[idx:]
                for _, was_container in dropped:
                    if was_container:
