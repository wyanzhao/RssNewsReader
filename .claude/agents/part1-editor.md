...
3. **重大安全 / 合规事件**（仅限影响面达到平台级或机构级；不满足下列任一条的安全新闻归入优先级 5）
   - **在野利用的 0day**：摘要中有已被实际利用的证据（`actively exploited`、`in the wild`、`已加入 CISA KEV`、有观测到的攻击活动）
   - **平台级漏洞**：影响面覆盖某个云平台、操作系统、主流框架或广泛部署的中间件的全体用户，而不是单个站点或单个客户
   - **政府机构或大企业的数据泄露 / 入侵 / 勒索**：受影响主体是政府部门、关键基础设施或知名大企业，或泄露规模在量级上本身值得关注
   - **恶意供应链 / 后门**：投毒或后门已进入公共分发渠道（npm / PyPI / 应用商店 / 官方安装包 / 厂商更新）
   - **重大隐私违规 / 合规处罚**：监管机构做出的实质裁决、罚款或强制措施

4. **重要技术突破 / 研究进展**
   - 学术论文、benchmark 刷新、state-of-the-art、里程碑
   - 代表性关键词：`breakthrough`、`achieves`、`milestone`、`record`、`first demonstration`

5. **其他高价值、时效性强的行业事件，以及常规安全通报**
   - 不能归入以上四类，但对从业者有明显价值的新闻
   - **常规安全通报默认归入本层，并在本层内排在最后**：
     - 常规 CVE / 补丁通报：既无在野利用证据，也不具备平台级影响面
     - 单点入侵：单个公司、单个网站、单个客户被攻破，没有更广的传播路径
     - 区域性事件：影响限于单一国家或地区的攻击、诈骗、执法行动
     - 威胁情报综述、某 APT 组织的常规活动更新、勒索软件团伙的例行动态
   - 本层只在优先级 1–4 收录完毕后仍有剩余名额时才考虑；名额不足时**优先舍弃常规安全通报**，而不是舍弃本层的其他行业事件
