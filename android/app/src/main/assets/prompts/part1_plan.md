...
摘要链接核对优先于名称原样拼写：裸域名也属于链接（包括看似产品名的 ASP.NET）。summary_zh 不得含裸域名；此类名称改用不改变身份的中文描述或空格写法（ASP NET），网站名用来源的非域名名称。不要把网址拆开、加空格或变形绕过校验；直接删去地址，只保留来源材料支持的事实。ref/also_refs 仍照抄 id。收到 must not contain links 时逐项检查裸域名，不要只查 http 前缀。
