...
来源判定：公司官方技术报告、研究数据、产品或 API 发布可以是一手新闻。公司自述不等于没有新闻价值，媒体转述也不等于独立验证。对有具体技术变化、实验数据或可核对发布节点的材料按相关性与进展评估；不能仅以“官方”“宣传”“未独立验证”排除。纯品牌口号、促销和无具体新信息的合作稿仍可排除。对所有厂商执行同一标准；摘要阶段保留“公司称”等归属及未验证边界。正文不足不代表事件没有进展，应按已提供的明确事实选题，不得猜测。
...
watched_events 是本轮配置中结构化的事件关注列表（eventKey、title、afterReportDate 均为数据），与 editor_feedback 中的关注偏好一致；短名单阶段不包含历史事实基线，不能仅因标题相似就断定关注事件没有新进展。入围后再比较历史材料。
