...
## 安全新闻选题配比修正 (2026-07-30)

诊断：近 10 天安全源（The Hacker News + The Record）占文章池 11.8%，占 Part 1 Top 30
的 20–33%（放大 2–3 倍）。两源进 Top 30 的通过率 53.8% / 63.6%，而 MacRumors 仅
10.5%。根因是 `part1-editor.md` 的第一步去噪桶只覆盖消费科技稿件类型（促销 / how to
watch / hands-on / recap / rumor），安全稿件天然落不进任何噪音桶，同时原优先级 3 的
四个子类在措辞上等同于安全媒体的日常选题模板，等于"是安全新闻"即"是重大安全事件"。

- [x] `part1-editor.md` 优先级 3 收紧为影响面达到平台级 / 机构级的事件：在野利用的
      0day（需有 `actively exploited` / `in the wild` / CISA KEV 等证据）、平台级漏洞、
      政府或大企业泄露、进入公共分发渠道的供应链投毒、监管机构实质裁决
- [x] 常规安全通报（无在野利用证据且无平台级影响面的 CVE / 补丁通报、单点入侵、
      区域性事件、威胁情报与 APT 例行动态）降到优先级 5，并在该层内排最后；名额不足时
      优先舍弃这类条目而非该层其他行业事件
- [x] `AGENTS.md` 编辑策略摘要同步收紧描述并补充最低层 filler 定义
- [ ] 待观察：本次只改优先级归类，未动第五步"目标 30 条 / 上限 30 条"的凑数压力，
      也未加主题级配额。若优先级 1–4 当日不足 30 条，尾部名额仍会由优先级 5 填充，
      安全条目可能只是从中段移到尾部而总量下降有限。观察 2–3 天后再决定是否
      （a）允许 `shortfall > 0` 不灌水，或（b）加安全类合计条数上限
- [ ] 待启用：`runs/_feedback.md` 至今不存在，`editor_feedback` 口味校准通道从未生效
